blackjack.controller('MainCtrl', function($scope, Card, blackjackRules, $timeout, $localStorage, $sessionStorage,ActionOption) {
    ...
    // $scope.$watchCollection('playerHands', function(newValue, oldValue, scope) {
    //   if(newValue != oldValue){
    //     for (var i = 0; i < newValue; i++) {
    //       scope.$watch('playerHands[i].cards.length', function(newcards, oldcards, scope){
    //           if(newcards != oldcards) {
    //               if (scope.playerHands[i].cards.length == 2) {
